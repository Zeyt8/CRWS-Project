...
partial struct UnitAnimateSystem : ISystem
{
    public void OnUpdate(ref SystemState state)
    {
        ...
        foreach ((LocalTransform transform, UnitAnimatorReference animatorReference, MovementData mov) in
                 SystemAPI.Query<LocalTransform, UnitAnimatorReference, MovementData>())
        {
            ...
            animatorReference.Value.SetFloat("Velocity", mov.CurrentVelocity);
        }

        foreach ((UnitAnimatorReference animatorReference, MagicAttackData attack) in
                 SystemAPI.Query<UnitAnimatorReference, MagicAttackData>())
        {
